...
DriverSeat2 = class( Seat )
DriverSeat2.maxChildCount = 20
DriverSeat2.connectionOutput = sm.interactable.connectionType.seated + sm.interactable.connectionType.power + sm.interactable.connectionType.bearing
DriverSeat2.colorNormal = sm.color.new( 0x80ff00ff )
DriverSeat2.colorHighlight = sm.color.new( 0xb4ff68ff )

local SpeedPerStep = 1 / math.rad( 27 ) / 3

DriverSeat2.Levels = {
	[tostring(obj_scrap_DriverSeat2)] = { maxConnections = 4, allowAdjustingJoints = false },
	[tostring(obj_interactive_DriverSeat2_01)] = { maxConnections = 6, allowAdjustingJoints = false, upgrade = obj_interactive_DriverSeat2_02, cost = 1, title = "#{LEVEL} 1" },
	[tostring(obj_interactive_DriverSeat2_02)] = { maxConnections = 8, allowAdjustingJoints = false, upgrade = obj_interactive_DriverSeat2_03, cost = 2, title = "#{LEVEL} 2" },
	[tostring(obj_interactive_DriverSeat2_03)] = { maxConnections = 12, allowAdjustingJoints = false, upgrade = obj_interactive_DriverSeat2_04, cost = 3, title = "#{LEVEL} 3" },
	[tostring(obj_interactive_DriverSeat2_04)] = { maxConnections = 16, allowAdjustingJoints = false, upgrade = obj_interactive_DriverSeat2_05, cost = 5, title = "#{LEVEL} 4" },
	[tostring(obj_interactive_DriverSeat2_05)] = { maxConnections = 20, allowAdjustingJoints = true, title = "#{LEVEL} 5" },
	[tostring(mod_driver_seat_vertical)] = { maxConnections = 40, allowAdjustingJoints = true, title = "#{LEVEL} 5" },
}

function DriverSeat2.server_onCreate( self )
	Seat:server_onCreate( self )
end

function DriverSeat2.server_onFixedUpdate( self )
...
function DriverSeat2.client_onInteract( self, character, state )
	if state then
		self:cl_seat()
...
function DriverSeat2.client_canInteractThroughJoint( self )
...
function DriverSeat2.client_onInteractThroughJoint( self, character, state, joint )
	local level = self.Levels[tostring( self.shape:getShapeUuid() )]
...
function DriverSeat2.client_onAction( self, controllerAction, state )
	--00Fant
...
--00Fant
function DriverSeat2.SetKeyParameter( self, data )
...
function DriverSeat2.sv_SetKeyParameter( self, data )
	local ID = self.shape:getId()
	if KeyParameter[ID] == nil then
		KeyParameter[ID] = {}
	end
...
function DriverSeat2.cl_SetKeyParameter( self, data )
	local ID = self.shape:getId()
	if KeyParameter[ID] == nil then
		KeyParameter[ID] = {}
	end
...
function DriverSeat2.server_onDestroy( self )
	local ID = self.shape:getId()
	KeyParameter[ID] = nil
end

function DriverSeat2.client_onDestroy( self )
	local ID = self.shape:getId()
	KeyParameter[ID] = nil
end
--00Fant

function DriverSeat2.client_getAvailableChildConnectionCount( self, connectionType )
...
function DriverSeat2.client_onCreate( self )
...
function DriverSeat2.client_onFixedUpdate( self, timeStep )
...
function DriverSeat2.client_onUpdate( self, dt )
...
function DriverSeat2.cl_onLeftAngleChanged( self, sliderName, sliderPos )
	self.cl.updateSettings.leftAngle = sliderPos + 1
	self.cl.updateDelay = 0.1
end

function DriverSeat2.cl_onRightAngleChanged( self, sliderName, sliderPos )
	self.cl.updateSettings.rightAngle = sliderPos + 1
	self.cl.updateDelay = 0.1
end

function DriverSeat2.cl_onLeftSpeedChanged( self, sliderName, sliderPos )
	self.cl.updateSettings.leftSpeed = ( sliderPos + 1 ) * SpeedPerStep
	self.cl.updateDelay = 0.1
end

function DriverSeat2.cl_onRightSpeedChanged( self, sliderName, sliderPos )
	self.cl.updateSettings.rightSpeed = ( sliderPos + 1 ) * SpeedPerStep
	self.cl.updateDelay = 0.1
end

function DriverSeat2.cl_onLockButtonClicked( self, buttonName )
	self.cl.updateSettings.unlocked = buttonName == "Off"
	self.cl.updateDelay = 0.1
end

function DriverSeat2.cl_onGuiClosed( self )
	if self.cl.updateDelay > 0.0 then
		self:cl_applyBearingSettings()
		self.cl.updateSettings = {}
		self.cl.updateDelay = 0.0
...
function DriverSeat2.cl_applyBearingSettings( self )
...
function DriverSeat2.cl_updateBearingGuiValues( self )
...
DriverSaddle = class( DriverSeat2 )
DriverSaddle.Levels = {
