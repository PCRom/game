...
function TapebotUnit.server_onProjectile( self, hitPos, hitTime, hitVelocity, projectileName, attacker, damage )
	...
	if type( attacker ) == "Player" or ( type( attacker ) == "Shape" and not isTrapProjectile( projectileName ) ) or teamOpponent then
		if damage > 0 then
...
function TapebotUnit.sv_spawnParts( self, impact, headDestroyed )
	...
	local headShape
	if self.explosiveProjectile then
		headShape = headBody:createPart( obj_robotparts_redtapebothead, sm.vec3.new( 0, 1, 2 ), sm.vec3.new( 0, 1, 0 ), sm.vec3.new( -1, 0, 0 ), true )
	else
		headShape = headBody:createPart( obj_robotparts_tapebothead01, sm.vec3.new( 0, 1, 2 ), sm.vec3.new( 0, 1, 0 ), sm.vec3.new( -1, 0, 0 ), true )
	end
	...
	headShape:setColor( color )
	middleShape:setColor( color )
	leftArmShape:setColor( color )
